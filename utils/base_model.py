...
def calculate_solar_output(latitude, longitude, system_power_kw, user_tilt):
    ...
    # ------------------------------------------------------------
    # Підхід для визначення оптимального кута:
    # Ітерація: Код перебирає кожен можливий кут нахилу (t) від 0° до 90°.
    # АОІ (Кут Падіння): Для кожного кута, щогодини протягом року, розраховується кут падіння сонячних променів (AOI) на площину панелі.
    # Проекція: Використовується np.cos(np.radians(aoi)) для визначення того, яка частина сонячного випромінювання (GHI) потрапляє безпосередньо на панель (POA).
